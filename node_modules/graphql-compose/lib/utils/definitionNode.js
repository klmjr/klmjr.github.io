...
function getArgumentNodes(data) {
...
function getDirectiveNodes(values) {
...
function getTypeNode(atc) {
  if (atc instanceof _ThunkComposer.ThunkComposer) {
    return getTypeNode(atc.ofType);
  } else if (atc instanceof _ListComposer.ListComposer) {
  ...
  } else if (atc instanceof _NonNullComposer.NonNullComposer) {
    const subType = getTypeNode(atc.ofType);
    if (!subType) return;
    return {
      kind: 'NonNullType',
